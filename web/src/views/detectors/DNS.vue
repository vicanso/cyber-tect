...
.dns
  ...
  el-card(
    ...
    template(
      #header
    ) {{ modifiedItem.id ? "修改DNS配置" : "添加DNS配置"}}
    el-form(
      label-width="90px"
      ref="form"
    ): el-row(
      ...
      el-col(
        ...
        placeholder="请输入DNS服务器IP，用,分割"
        v-model="modifiedItem.servers"
        clearable
      )
      //- 配置状态
      el-col(
...
export default defineComponent({
  ...
  setup() {
    ...
    return {
      ...
      dnses: detectorStore.state.dnses,
      listDNS: (params) => detectorStore.dispatch("listDNS", params),
      addDNS: (params) => detectorStore.dispatch("addDNS", params),
      updateDNSByID: (params) =>
        detectorStore.dispatch("updateDNSByID", params),
    };
  },
  data() {
    return {
  ...
  methods: {
    ...
    async submit(): Promise<boolean> {
      let isSuccess = false;
      const { modifiedItem, dnses, originalItem } = this;
      if (dnses.processing) {
        return isSuccess;
      }
      const item = this.convertToDNS(modifiedItem);
      try {
        // 添加
        if (!item.id) {
          await this.addDNS(item);
        } else {
          const updateData = diff(item, originalItem);
          if (updateData.modifiedCount === 0) {
            throw new Error("请先修改再提交");
          }
          await this.updateDNSByID({
            id: item.id,
